...
[#X003]
== {SHOULD} Resort to Query-by-Example Query Parameters If Needed
...
assess if query-by-example using query parameters doesn't meet the
requirements. If so, name corresponding query parameters according to
https://github.com/allegro/restapi-guideline#simple-filters-in-query-string[Allegro's guideline].

////
We can embed relevant fragment of the Allegro guidelines directly here or leave
the link.
