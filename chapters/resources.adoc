...
[#X002]
== {MAY} Group Resources Under a Prefix

API designers are allowed to place endpoints allowing access to different
resources under a collective, dedicated prefix, provided that all these
resources constitute a big, overarching product of the platform that has been
identified by the product team.

For example `/financial-timeline` can be used as a prefix for all resources from
the financial domain, forming the "financial timeline" product.

////
Comments:
Wilerson: There's domain-level grouping in that case, no?
////
...
////
Comments:
Wilerson: Would it be possible to allow for duplication across different
    gateways? (not sure this'd be desirable)
Andrzej: @Wilerson - I think that would not be desirable.
////
